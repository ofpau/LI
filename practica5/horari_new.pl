...
% We introduce the following variable for efficiency :)
...
eachCourse3Hours:- course(C), findall(courseDay-C-D, day(D), Lits), exactly(3, Lits), fail.
eachCourse3Hours.

eachStudentAtMost3HoursPerDay:- day(D), student(_, L), findall(courseDay-C-D, member(C, L), Lits), atMost(3, Lits), fail.
eachStudentAtMost3HoursPerDay.
...
defineCourseDay:- day(D), course(C), findall(courseHour-C-D-H, hour(H), Lits), expressOr(courseDay-C-D, Lits), fail.
defineCourseDay.
...
main:-  run(5), nl, write("No solution found for this problem."), nl, halt.
...
treatResult(10, MAX):- write('Solution found with '), write(MAX), write(' "late days".'), nl,
  ...
  %% Uncomment to see every generated solution:
  %%displaySol(M), 
  NEWMAX is MAX-1, run(NEWMAX),
  nl, displaySol(M),
  nl, nl, write('The solution found with minimum "late days" has '), 
  write(MAX), write(' '), write('day of class at 13.00h'),
  nl, nl, halt.
